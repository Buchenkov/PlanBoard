...
# # helpers: иконка приложения + AppUserModelID на Windows

# def build_app_icon():
#     # Приоритет — .ico (Windows)
#     candidates = [
#         ":/icons/app.ico",
#         ":/icons/planboard.ico",
#         "app/resources/icons/app.ico",
#         "app/resources/icons/planboard.ico",
#         ":/icons/app.png",
#         ":/icons/planboard.png",
#         "app/resources/icons/app.png",
#         "app/resources/icons/planboard.png",
#         ":/icons/app.svg",
#         ":/icons/planboard.svg",
#         "app/resources/icons/app.svg",
#         "app/resources/icons/planboard.svg",
#     ]
#     for p in candidates:
#         icon = QtGui.QIcon(p)
#         if not icon.isNull():
...
# def set_win_appusermodel_id(app_id="YourCompany.PlanBoard"):
...
# class TitleBar(QtWidgets.QWidget):
#     sigCloseClicked = QtCore.pyqtSignal()  # внешний сигнал "закрыть"
#     height_hint = 36
...
#         self.btn_min.clicked.connect(self._on_minimize)
#         self.btn_max.clicked.connect(self._on_max_restore)
#         # ВАЖНО: не используем self.titlebar.* внутри TitleBar
#         # Кнопка закрытия испускает сигнал; конкретное действие решает окно-владелец
#         self.btn_close.clicked.connect(self.sigCloseClicked.emit)
...
#     def _window(self):
#         # Упрощённый доступ к окну-владельцу
#         return self.window()
...
#     print("FramelessWindow.__init__ start")

#     """
#     Окно без системной рамки со скруглениями, тенью, собственным TitleBar
#     и сохранением геометрии через QSettings.
#     Обновление иконки Max/Restore выполнено через changeEvent (без windowStateChanged).
#     """
#     RADIUS = 10
#     SHADOW_BLUR = 30
#     SHADOW_OFFSET_Y = 8
...
#         # TitleBar
#         self.titlebar = TitleBar(self, title="Планировщик задач")
#         # ожидается, что в TitleBar есть сигналы и метод _on_max_restore/_apply_icons
#         if hasattr(self.titlebar, "sigCloseClicked"):
#             self.titlebar.sigCloseClicked.connect(self._on_close_clicked)

#         # Центральная область
#         self._central = QtWidgets.QWidget(self.frame)
#         self._central.setObjectName("CentralArea")
#         self._central.setAutoFillBackground(False)
#         self._central.setMouseTracking(True)
#         self._central_layout = QtWidgets.QVBoxLayout(self._central)
#         self._central_layout.setContentsMargins(0, 0, 0, 0)
#         self._central_layout.setSpacing(0)

#         inner_lay = QtWidgets.QVBoxLayout(self.frame)
#         inner_lay.setContentsMargins(0, 0, 0, 0)
#         inner_lay.setSpacing(0)
#         inner_lay.addWidget(self.titlebar)
#         inner_lay.addWidget(self._central, 1)

#         # Внешняя компоновка (прозрачные поля для тени)
#         wrap = QtWidgets.QVBoxLayout(self)
#         wrap.setContentsMargins(10, 10, 10, 10)
#         wrap.setSpacing(0)
#         wrap.addWidget(self.frame)

#         # Ресайз-состояния
#         self._resizing = False
#         self._resize_edge = None
#         self._press_pos_global = QtCore.QPoint()
#         self._press_geo = QtCore.QRect()

#         # Восстановление геометрии
#         self._restore_geometry_or_center()

#     # Публичный API: центральный виджет
#     def setCentralWidget(self, w: QtWidgets.QWidget):
#         for i in reversed(range(self._central_layout.count())):
#             item = self._central_layout.takeAt(i)
#             wid = item.widget()
#             if wid is not None:
#                 wid.setParent(None)
#         if w is not None:
...
#     # Закрытие приложения целиком
#     def _on_close_clicked(self):
#         QtWidgets.QApplication.instance().quit()

#     # Восстановление геометрии
#     def _restore_geometry_or_center(self):
#         geometry = self.settings.value("main/geometry", type=QtCore.QByteArray)
#         state = self.settings.value("main/windowState", type=int)
#         if geometry:
...
#         if state == int(QtCore.Qt.WindowMaximized):
#             self.showMaximized()
...
#             self.settings.setValue("main/windowState", int(self.windowState()))
#         except Exception:
#             pass
#         super().closeEvent(e)

#     # Отслеживаем смену состояния окна без сигнала windowStateChanged
#     def changeEvent(self, e: QtCore.QEvent):
#         if e.type() == QtCore.QEvent.WindowStateChange:
#             if hasattr(self.titlebar, "_apply_icons"):
#                 self.titlebar._apply_icons()
#         super().changeEvent(e)

#     def showEvent(self, e: QtGui.QShowEvent):
#         # При первом показе тоже обновим иконки
#         if hasattr(self.titlebar, "_apply_icons"):
#             self.titlebar._apply_icons()
#         super().showEvent(e)

#     # Двойной клик по пустой зоне — Max/Restore
#     def mouseDoubleClickEvent(self, e: QtGui.QMouseEvent):
#         if e.button() == QtCore.Qt.LeftButton and not self._hit_test_resize_edge(e.pos()):
#             if hasattr(self.titlebar, "_on_max_restore"):
...
#     def mousePressEvent(self, e: QtGui.QMouseEvent):
#         if e.button() == QtCore.Qt.LeftButton:
#             edge = self._hit_test_resize_edge(e.pos())
...
#     def mouseReleaseEvent(self, e: QtGui.QMouseEvent):
#         if e.button() == QtCore.Qt.LeftButton and self._resizing:
#             self._resizing = False
#             self._resize_edge = None
#             e.accept()
#             return
...
#     # Хит-тест краёв
#     def _hit_test_resize_edge(self, pos: QtCore.QPoint):
#         if self.isMaximized():
#             return None

#         frame_pos = self.frame.mapFrom(self, pos)
#         fr = self.frame.rect()
#         if not fr.contains(frame_pos):
#             return None

#         x, y, w, h = frame_pos.x(), frame_pos.y(), fr.width(), fr.height()
...
#     def _perform_resize(self, global_pos: QtCore.QPoint):
#         if not self._resize_edge or self.isMaximized():
#             return

#         delta = global_pos - self._press_pos_global
#         geo = QtCore.QRect(self._press_geo)

#         minw = max(self.minimumWidth(), 300)
#         minh = max(self.minimumHeight(), 200)
...
#             new_right = max(new_right, geo.left() + minw)
...
#             new_bottom = max(new_bottom, geo.top() + minh)
#             geo.setBottom(new_bottom)

#         screen = self.screen() or QtWidgets.QApplication.primaryScreen()
#         if screen:
#             avail = screen.availableGeometry().adjusted(5, 5, -5, -5)
#             # Можно внедрить мягкие ограничения в пределах экрана при желании
#             # Например, не давать полностью уходить вверх/влево
#             if geo.right() < avail.left():
#                 geo.moveRight(avail.left())
#             if geo.bottom() < avail.top():
#                 geo.moveBottom(avail.top())

#         self.setGeometry(geo)
        
#     print("FramelessWindow.__init__ done")
